#!/usr/bin/env python3
"""
SASRec baseline matched to the current CoMemTree evaluation protocol.
...
1) Use the SAME evaluation users as CoMemTree:
   ...
   Behavior text is NEVER used by SASRec.

2) Train ONLY from each selected user's:
       train[-history_size:]
   with history_size=10 by default.
   No validation/test interaction is used in fitting.

3) Training objective:
   causal next-item prediction on TRAIN sequences only:
       [i1] -> i2
       [i1,i2] -> i3
       ...
   implemented as a causal Transformer with full-softmax item prediction.

4) Evaluation:
       input = train[-history_size:]
   Use the hidden state of the final observed TRAIN item to score exactly the
   SAME candidate set as CoMemTree:
       test_ids + test_neg
...
5) Train-time item vocabulary:
   ...
   Validation/test/candidate IDs never expand the vocabulary before fitting.
   Evaluation candidates outside this vocabulary are OOV and receive -inf.

6) No early stopping or tuning on test metrics.

Outputs
-------
<output_dir>/
  sasrec_metrics.json
  sasrec_rankings.jsonl
  fixed_candidates_snapshot.json
  sasrec_model.pt

Example
-------
CUDA_VISIBLE_DEVICES=1 python sasrec_comemtree_baseline.py \
  ...
  --lr 1e-3 \
  --weight-decay 0.0 \
  --output-dir results/sasrec_cd_300
"""
...
def select_users(
    ...
    if requested is not None:
        ...
        # Preserve sequence-file order, matching current CoMemTree.
        users = [
            uid
...
class SASRecTrainDataset(Dataset):
    """
    Each record is one whole TRAIN sequence, encoded as real item IDs:
        1..N
    Padding ID = 0.
    ...
    The collator constructs:
        input  = seq[:-1]
        target = seq[1:]
    so every target is an observed TRAIN interaction.
    """
...
class SASRecCollator:
    ...
    def __call__(
        ...
        for b, seq in enumerate(batch):
            # Need at least two items: current item -> next item.
...
class SASRec(nn.Module):
    ...
    def next_item_logits(
        ...
        """
        hidden: [..., H]
        returns logits over N real items.
        Weight tying with item embeddings rows 1:.
        """
        real_item_weights = (
...
def main(args: argparse.Namespace) -> None:
    ...
    # static items.json + allowed TRAIN item IDs only.
    # -----------------------------------------------------------------
    item_ids: List[str] = []
    ...
    with torch.no_grad():
        for uid in users:
            ...
            # Hidden state of the final observed training item predicts next item.
            final_hidden = hidden[
                0,
                L - 1,
            ]
            ...
            # Stable tie-break preserves CoMemTree candidate order.
            order = sorted(
