...
- Incomplete rankings are retried. Summary reports both all-user and
  parse-complete-only metrics.
...
RANK_PROMPT_VERSION = "what_plus_how_controlled_v1"
    ...
    def rank_jobs(
        ...
        for start in pbar:
            ...
            for local_idx, (
                ...
                # Retry only malformed/incomplete samples, individually.
                while (
                    not meta[
                        "parse_complete"
                    ]
                    ...
                    pbar.set_postfix(
                        users=f"{start + 1}-{end}/{len(jobs)}",
                        retry=f"sample{local_idx}:attempt{attempts + 1}",
                        refresh=True,
                    )
                    retry_prompt = (
                        prompt
                        + "\n\nIMPORTANT RETRY:\n"
                        + f"Your previous response did not contain all "
                        + f"{len(valid_ids)} candidate IDs exactly once. "
                        + "Return one complete JSON object containing EVERY "
