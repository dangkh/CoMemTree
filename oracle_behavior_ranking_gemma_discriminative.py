...
# Prompt versions are stored in cache/results so old Oracle behaviors/rankings
# are NOT silently reused after prompt changes.
BEHAVIOR_PROMPT_VERSION = "oracle_discriminative_behavior_v2"
ORACLE_RANK_PROMPT_VERSION = "oracle_behavior_primary_v2"
...
def behavior_prompt(
    ...
    """
    ORACLE diagnostic:
    the held-out GT item is intentionally visible ONLY in this behavior-
    construction stage.

    The generated behavior must preserve discriminative semantics of the
    next choice without copying its exact title or item ID.
    """
    return f"""You are constructing an ORACLE NEXT-BEHAVIOR signal for a recommendation diagnostic.

You are given:
1. the user's RECENT HISTORY;
2. the item the user ACTUALLY selected next.

Your task is NOT to summarize the user's general taste.
Your task is to describe the specific NEXT BEHAVIOR that explains the transition
from the recent history to the next selected item.

The behavior will later be used by a small ranking model to distinguish the next
choice from unrelated candidate items, so it must be CONCRETE and DISCRIMINATIVE.

WHAT TO PRESERVE WHEN SUPPORTED:
- genre or subgenre direction;
- style, mood, theme, or use case;
- soundtrack / film / game orientation;
- regional or cultural direction;
- live / compilation / collection-expansion pattern when meaningful;
- creator continuity ONLY when the recent history actually supports it;
- whether the user is deepening, broadening, or shifting from the recent history.

IMPORTANT:
- If recent history strongly supports the next item, state the concrete continuity.
- If the next item is only moderately related, describe the concrete transition.
- If the next item is weakly related to history, explicitly describe a SHIFT.
- Do NOT invent \"same creator\", \"same series\", or \"same category\" continuity when
  it is not supported by the recent history.
- Avoid generic phrases such as \"exploring related music\", \"expanding musical
  interests\", \"seeking similar sounds\", or \"continuing preferences\" when a more
  specific semantic description is possible.
- Prefer behavior descriptions that would eliminate clearly unrelated item types.

STRICT ANTI-LEAKAGE RULES:
- Do NOT output the target item_id.
- Do NOT copy or mention the exact target title.
- Do NOT mention candidate position, \"ground truth\", \"correct item\", or \"target item\".
- Do NOT simply paraphrase the target title.
- You MAY use genre/subgenre/style/theme/category semantics implied by the next item.
- You MAY describe a creator/artist continuity only if that continuity is actually
...
Assess how much the RECENT HISTORY supports the actual next selection:
- strong: clear same/adjacent semantic trajectory;
- moderate: plausible bridge but not direct continuity;
- weak: little direct support, so the behavior should be framed as a shift.

Return JSON only:
{{
  \"history_relation\": \"strong|moderate|weak\",
  \"history_relation_reason\": \"one concise sentence describing the semantic bridge from history, without the exact next-item title\",
  \"behavior_signature\": \"short, concrete, discriminative phrase\",
  \"mechanism\": \"one broad label\",
  \"scope\": \"one broad label\",
  \"direction\": \"one broad label\",
  \"next_behavior_description\": \"one concise, concrete sentence describing what kind of item the user is moving toward next\"
}}
"""
...
def normalize_oracle_behavior(obj: Any) -> Dict[str, str]:
    ...
    history_relation = clean_text(obj.get("history_relation")).lower()
    if history_relation not in {"strong", "moderate", "weak"}:
        history_relation = "unknown"

    out = {
        "history_relation": history_relation,
        "history_relation_reason": clean_text(obj.get("history_relation_reason")),
...
def format_history_for_prompt(items: List[Dict[str, str]]) -> str:
    """Compact one-line history representation for Gemma 3 4B."""
    lines = []
    for i, x in enumerate(items, 1):
        lines.append(f"{i}. {x['title']} | {x['category']}")
    return "\n".join(lines)


def format_candidates_for_prompt(items: List[Dict[str, str]]) -> str:
    """Compact one-line candidate representation; input order is random."""
    lines = []
    for i, x in enumerate(items, 1):
        lines.append(f"[{i}] {x['item_id']} | {x['title']} | {x['category']}")
    return "\n".join(lines)
...
def oracle_rank_prompt(
    ...
    # For Gemma 3 4B, expose only natural-language behavior fields.
    # mechanism/scope/direction remain in the cache for analysis, but are not
    # extra instructions for the ranking model.
    behavior_signature = oracle_behavior["behavior_signature"]
    behavior_description = oracle_behavior["next_behavior_description"]

    history_text = format_history_for_prompt(history_items)
    candidate_text = format_candidates_for_prompt(candidate_items)

    return f"""You are ranking candidate items for the user's NEXT choice.

PRIMARY SIGNAL — PREDICTED NEXT BEHAVIOR
The NEXT BEHAVIOR below is assumed to be a reliable description of what the user
is moving toward next. Treat it as the PRIMARY signal for ranking.

Behavior: {behavior_signature}
Description: {behavior_description}

SECONDARY CONTEXT — RECENT USER HISTORY
Use recent history to interpret the NEXT BEHAVIOR and to break ties between
candidates that fit the behavior similarly.
Do NOT infer a different next behavior from the history.
Do NOT let a candidate that matches only the history outrank a candidate that
clearly matches the provided NEXT BEHAVIOR.

Recent history (oldest to newest):
{history_text}

CANDIDATE ITEMS
The candidate input order is RANDOM and has no ranking meaning.

{candidate_text}

DECISION RULE
1. Translate the NEXT BEHAVIOR into concrete semantic characteristics such as
   genre/subgenre, style, theme, soundtrack orientation, region, mood, creator
   continuity when explicitly supported, or a deepening/broadening/shift direction.
2. Rank candidates that DIRECTLY instantiate those characteristics highest.
3. Among similarly behavior-matching candidates, use RECENT HISTORY as a tie-breaker.
4. Rank all remaining candidates by decreasing compatibility.

IMPORTANT:
- Behavior match is PRIMARY.
- History is SECONDARY.
- Candidate input order must not influence the ranking.
- Base matching on title/category semantics that are actually shown.
- Rank ALL {len(ids)} candidate IDs exactly once.
...
Return JSON only:
{{"ranked_item_ids":["item_id_1","item_id_2","..."],"reasoning":"one short sentence explaining the behavior match used for the top-ranked candidates"}}
"""
...
def load_behavior_cache(
    path: Path,
    required_prompt_version: str = BEHAVIOR_PROMPT_VERSION,
) -> Dict[str, Dict[str, Any]]:
    """Reuse only cache rows generated by the CURRENT behavior prompt."""
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                row = json.loads(line)
                if (
                    row.get("oracle_ok") is True
                    and row.get("behavior_prompt_version") == required_prompt_version
                ):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--behavior-max-new-tokens", type=int, default=256)
...
def main() -> None:
    ...
    print(f"behavior prompt ver   : {BEHAVIOR_PROMPT_VERSION}")
    print(f"oracle rank prompt ver: {ORACLE_RANK_PROMPT_VERSION}")
    ...
    if args.resume and output.exists():
        try:
            ...
            if isinstance(old, list):
                results = [
                    x for x in old
                    if x.get("behavior_prompt_version") == BEHAVIOR_PROMPT_VERSION
                    and x.get("oracle_rank_prompt_version") == ORACLE_RANK_PROMPT_VERSION
                ]
                completed = {str(x["user_id"]) for x in results}
                print(
                    f"Resume ranking: {len(completed)} completed users "
                    f"for current prompt versions"
                )
        except Exception as e:
    ...
    for start in range(0, len(pending), args.batch_size):
        ...
        for idx, uid in enumerate(batch_users):
            ...
            row = {
                "user_id": uid,
                "behavior_prompt_version": BEHAVIOR_PROMPT_VERSION,
                "oracle_rank_prompt_version": ORACLE_RANK_PROMPT_VERSION,
                "target": target,
                ...
                "history_relation_to_gt": behavior_cache[uid]["oracle_behavior"].get("history_relation"),
                "history_relation_reason": behavior_cache[uid]["oracle_behavior"].get("history_relation_reason"),
    ...
    summary = {
        ...
        "history_size": args.history_size,
        "history_to_gt_relation_counts": relation_counts,
